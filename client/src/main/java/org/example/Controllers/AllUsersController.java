...
import org.example.Helpers.SessionManager;
...
public class AllUsersController {
    ...
    private FriendRequestsService requestService = new FriendRequestsService();
    ...
    private Node createUserRow(int userId, String username, String status) {
        ...
        switch (status) {
            case "FRIEND":
                ...
                action.setOnMouseClicked(e -> unfriendUser(userId,currentUser.getUserId()));
                break;
            ...
            default:
                ...
                action.setStyle("-fx-text-fill: #2ecc71; -fx-font-size: 24; -fx-font-weight: bold; -fx-cursor: hand;");
                action.setOnMouseClicked(e -> addfriendUser(userId, currentUser.getUserId()));
        }
    ...
    private void unfriendUser(int userId, int curentID){
        service.unfriend(userId, curentID);
        initialize();
    }

    private void sendRequest(int userId,int curentID){
        requestService.acceptRequest(userId, curentID);
        initialize();
    }

    private void addfriendUser(int userId, int curentID){
        service.sendFriendRequest(userId, curentID);
        initialize();
    }
}
